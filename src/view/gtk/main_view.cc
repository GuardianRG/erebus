#include <gtkmm.h>

#include <main_view.h>
#include <string>
#include <assert.h>

#include <interfaces/i_main_view.h>
#include <interfaces/i_presenter.h>

namespace erebus {
	
MainView::MainView(BaseObjectType* cobject, const Glib::RefPtr<Gtk::Builder>& refBuilder) : Gtk::Window(cobject) {
	refBuilder->get_widget("menu_file_open",menu_file_open_);
	menu_file_open_->signal_activate().connect(sigc::mem_fun(*this,&MainView::on_menu_file_open_click));
	
	
	setPreferredSize(800,600);
	maximize();
}

MainView::~MainView() {

}

void MainView::on_menu_file_open_click() {
	Gtk::FileChooserDialog dialog("Please choose a file",
				      Gtk::FILE_CHOOSER_ACTION_OPEN);
	dialog.set_transient_for(*this);
	
	//Add response buttons the the dialog:
	dialog.add_button("_Cancel", Gtk::RESPONSE_CANCEL);
	dialog.add_button("_Open", Gtk::RESPONSE_OK);
	
	int result = dialog.run();
	
	//Handle the response:
	if(result==Gtk::RESPONSE_OK) {
			presenter_->loadFile(dialog.get_filename());
	}
	
}

void MainView::maximize() {
	Gtk::Window::maximize();
}

void MainView::unmaximize() {
	Gtk::Window::unmaximize();
}

void MainView::setPreferredSize(int width,int height) {
	set_default_size(width,height);
}

void MainView::setTitle(std::string title) {
	set_title(title);
}

void MainView::setPresenter(IPresenter* presenter) {
	presenter_=dynamic_cast<IMainViewPresenter*>(presenter);
	assert(presenter_!=nullptr);
}

}